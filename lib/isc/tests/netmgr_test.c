...
nm_setup(void **state __attribute__((unused))) {
	...
	for (size_t i = 0; i < MAX_NM; i++) {
		nm[i] = isc_nm_start(test_mctx, workers);
		assert_non_null(nm[i]);
		/* The arguments are in tenths of seconds */
		isc_nm_settimeouts(nm[i], T_INIT / 100, T_IDLE / 100,
				   T_KEEPALIVE / 100, T_ADVERTISED / 100);
	}
...
nm_teardown(void **state __attribute__((unused))) {
	...
	while ((isc_refcount_current(&active_cconnects) != 0) ||
	       (isc_refcount_current(&active_csends) != 0) ||
	       (isc_refcount_current(&active_creads) != 0) ||
	       (isc_refcount_current(&active_ssends) != 0) ||
	       (isc_refcount_current(&active_sreads) != 0))
	{
		usleep(1000);
	}
...
static isc_result_t
udp_connect(isc_nm_t *connect_nm) {
	return (isc_nm_udpconnect(connect_nm,
...
static isc_result_t
tcp_connect(isc_nm_t *connect_nm) {
	return (isc_nm_tcpconnect(connect_nm,
...
static isc_result_t
tcpdns_connect(isc_nm_t *connect_nm) {
	return (isc_nm_tcpdnsconnect(connect_nm,
				     (isc_nmiface_t *)&tcp_connect_addr,
				     (isc_nmiface_t *)&tcp_listen_addr,
				     connect_connect_cb, NULL, T_CONNECT, 0));
}
...
static isc_result_t
tlsdns_connect(isc_nm_t *connect_nm) {
	return (isc_nm_tlsdnsconnect(
		connect_nm, (isc_nmiface_t *)&tcp_connect_addr,
