...
tcp_recv(isc_nmhandle_t *handle, isc_result_t eresult, isc_region_t *region,
	...
	dispatch_log(disp, LVL(90), "got TCP packet: requests %d, recvs %d",
		     disp->requests, disp->recv_pending);
...
startrecv(dns_dispatch_t *disp, dispsocket_t *dispsock) {
	...
	case isc_socktype_udp:
		isc_nm_read(handle, udp_recv, dispsock);
		break;
...
dns_dispatch_removeresponse(dns_dispentry_t **resp,
	...
	*resp = NULL;
	...
	if (res->dispsocket != NULL) {
		if (res->dispsocket->handle != NULL) {
			isc_nmhandle_detach(&res->dispsocket->handle);
		}
